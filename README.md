...
This is a guide which I wrote for myself when I had to install a similar enviroment on several machines. This instruction are quite shallow and probably will only be suitable for my case.
...
1. Go to vpn settings. Enter the gateway which your company provides. In the password form select that it will be applicable for all users otherwise on login you will be disconnected. (TUNI note: `staff-ras.vpn.tut.fi` gateway and the credentials from TUNI).
2. Click `Advanced`. There uncheck everything except for `MSCHAPv2` in authentication methods. Check `Use Point-to-Point encription (MPPE)`, `Allow BSD data comperssion`, `Allow Deflate data compression`, `Use TCP header compression` other boxes leave unchecked.
2. Then we need to prohibit Wired connections to connect automatically after the startup. Type `nm-connection-editor` in terminal (or just go to Edit connection on 16.04). Click on wired connections and go to Preferences (setting icon at the bottom) > General > uncheck the box. Repeat for another connection if you have more than two ports;
4. After the automatic connection is turned on, bind the VPN to this particular connections in the `nm-connection-editor` settings. So it will automatically connect to VPN after it is connected to the internet. TUNI note: useful command: `nmap -sn 130.230.89.0/24`, `nmcli con show`
